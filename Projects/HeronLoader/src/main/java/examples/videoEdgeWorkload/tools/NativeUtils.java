...
/**
 * A utility class used to load platform dependent (OpenCV) libraries and other resources and is
 * used by {@link OpenCVOp} implementations. Libraries must be present on the classpath for this
 * utility class to function properly. If the library resides within a jar file it will be extracted
 * to the local tmp directory before it is loaded.
 *
 * <p>The project is shipped with the following OpenCV 2.4.8 binaries:
 *
 * <ul>
 *   <li>Windows dll's both 32 and 64 bit
 *   <li>Mac OS dylib, 64 bit only
 *   <li>Ubuntu 12.04 LTS so 64 bit only
 * </ul>
 *
...
public class NativeUtils {
   ...
   */
  public static File extractTmpFileFromJar(String path, boolean deleteOnExit, String additionalID)
      throws IOException {

    // Prepare temporary file
    File temp = File.createTempFile("abcd", "efgh");
    temp.delete();
    // If we use full path, it breaks when subdirectories are used, fix
    // by unpacking everything to same directory
    File inputTemp = new File(path);
    temp =
        new File(
  ...
  public static File extractTmpFileFromJar(String path, boolean deleteOnExit) throws IOException {
    return extractTmpFileFromJar(path, deleteOnExit, "");
  }
}
