...
public class HelperRunner {
  ...
  public static void runTopology(String[] args, StormTopology topology, Config conf)
      throws Exception {
    ...
    if (args != null) {
      if (args.length < 2) {
        Exception exception =
            new IllegalArgumentException(
                "Illegal number of command line arguments supplied."
      ...
      if (!args[1].equals(MODE_OF_OPERATION_CLUSTER) && !args[1].equals(MODE_OF_OPERATION_LOCAL)) {
        Exception exception =
            new IllegalArgumentException(
                "The allowed values for the second argument is either 'Cluster' or 'Local'. Please"
                    + " provide a valid value for the second argument.");
        throw exception;
      }
  ...
  private static void runTopologyLocally(
      StormTopology topology, String topologyName, Config conf, int runtimeInSeconds)
      throws InterruptedException, AlreadyAliveException, InvalidTopologyException,
          NotAliveException {
